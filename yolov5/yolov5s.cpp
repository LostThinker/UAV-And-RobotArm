...
ICudaEngine *createEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt)
{
    ...
    std::cout<<"deconv11 done"<<std::endl;
    ...
    config->setMaxWorkspaceSize(160 * (1 << 20)); // 16MB
#ifdef USE_FP16
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "this cmd deleted" << std::endl;
    ...
    while (true)
    {
        ...
        else
        {
            ...
            auto &ress = batch_res[0];
            // std::cout << ress.size() << std::endl;
            for (size_t j = 0; j < ress.size(); j++)
            {
                cv::Rect r = get_rect(img, ress[j].bbox);
                cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
                cv::putText(img, std::to_string((int)ress[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            }
            cv::imshow("img", img);
            cv::waitKey(1);
        }
